...
router.post('/register', async (req, res) => {
  let { email, password, role, ref, name, idNumber, apartmentId, inviteCode } = req.body;
  // Accept new role aliases from frontend: map 'owner' -> 'landlord', 'property_manager' -> 'caretaker'
  if (role === 'owner') role = 'landlord';
  if (role === 'property_manager') role = 'caretaker';
  try {
    ...
    if (role === 'tenant') {
      ...
      if (!profileId) {
        ...
        if (!profileId) {
          // If landlord pre-created a Caretaker profile (by email), reuse it instead of creating duplicate
          let c = null;
          if (email) c = await models.Caretaker.findOne({ where: { email } });
          if (c) {
            // update association if not present
            const updates = {};
            if (!c.estateId && inv.estateId) updates.estateId = inv.estateId;
            if (!c.apartmentId && inv.apartmentId) updates.apartmentId = inv.apartmentId;
            if (Object.keys(updates).length) await c.update(updates);
            profileId = c.id;
            await inv.update({ usedAt: new Date() });
          } else {
            const nc = await models.Caretaker.create({ email, name: name || null, idNumber: idNumber || null, estateId: inv.estateId || null, apartmentId: inv.apartmentId || null });
            profileId = nc.id;
            await inv.update({ usedAt: new Date() });
          }
        }
