...
export default function AssignCaretaker({ token, estates, onAssigned }) {
  ...
  const submit = async (e) => {
    ...
    try {
      setLoading(true);
      if (!estateId || !caretakerId) throw new Error('Estate and caretaker are required');
  ...
  return (
    <form onSubmit={submit} className="p-4 bg-white rounded shadow space-y-2">
      <h4 className="text-lg font-medium">Assign Caretaker</h4>
      <select className="input" value={estateId} onChange={e=>setEstateId(e.target.value)}>
        <option value="">Select estate</option>
        {(estates||[]).map(e => <option key={e.id} value={e.id}>{e.name}</option>)}
      </select>
      <input className="input" placeholder="Caretaker ID" value={caretakerId} onChange={e=>setCaretakerId(e.target.value)} />
